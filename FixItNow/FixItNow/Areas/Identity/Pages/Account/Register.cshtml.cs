...
namespace FixItNow.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<MyApplicationUser> _userStore;
        private readonly IUserEmailStore<MyApplicationUser> _emailStore;
        ...
        {
            returnUrl ??= Url.Content("~/Authentication/SignIn");
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            if (ModelState.IsValid)
            {
                ...
                User temp = new User { firstName=user.FirstName, lastName=user.LastName,phoneNumber=user.PhoneNumber, address=user.Address,city=user.City,gender=user.Gender,password=Input.Password,email=user.Email,joinedTime=DateTime.Now };
                Credentials c1= new Credentials { email=temp.email ,password=temp.password,type="user"};
                c.Userss.Add(temp);
                c.Credentialss.Add(c1);
                c.SaveChanges();

                var newClaim = new Claim("department", "cs");
            ...
            //}
            //Credentials cred = new Credentials { email =Input.Email, password = Input.Password, type = "user" };
            return Page();
        }
        ...
        private IUserEmailStore<MyApplicationUser> GetEmailStore()
        {
            ...
            return (IUserEmailStore<MyApplicationUser>)_userStore;
        }
    }
}
